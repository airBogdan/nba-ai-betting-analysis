...
Usage:
    ...
    python3 synth.py --forecast                 # Full forecast for all assets
...
def extract_metrics(data):
    ...
    if forecast_vol:
        ...
        if valid_forecast:
            ...
            result["vol_forecast_next"] = valid_forecast[0]
...
def print_comparison(assets_data):
    ...
    for ticker, data, metrics, _ in sorted_rows:
        ...
        forecast_str = f"{forecast:.1f}%" if forecast is not None else "N/A"
        level = vol_level(vol)
        emoji = vol_emoji(vol)
...
def monte_carlo_stats(paths):
    ...
    for step in range(steps):
        step_prices = sorted([p[step] for p in paths])
        n = len(step_prices)
        stats["p5"].append(step_prices[int(n * 0.05)])
        stats["p25"].append(step_prices[int(n * 0.25)])
        stats["p50"].append(step_prices[int(n * 0.50)])
        stats["p75"].append(step_prices[int(n * 0.75)])
        stats["p95"].append(step_prices[int(n * 0.95)])
...
def main():
    ...
    parser.add_argument("--forecast", action="store_true", help="All assets with forecast vs realized comparison")
    ...
    # Fetch data (rate limit when fetching multiple)
    assets_data = {}
    ...
    # Forecast: comparison table + per-asset detail
    if args.forecast:
        print_comparison(assets_data)
