...
from betting.analyze.gamedata import extract_game_id, format_matchup_string
...
from betting.analyze.sizing import (
    CONFIDENCE_WIN_PROB,
    KELLY_FRACTION,
...
from betting.names import names_match, normalize_name
from betting.prompts import compact_json
from betting.llm import _strip_markdown_json
...
class TestPipelineDuplicateCheck:
    ...
    @pytest.mark.asyncio
    @patch("betting.analyze.pipeline.get_active_bets")
    async def test_aborts_when_bets_exist_for_date(self, mock_active):
        from betting.analyze.pipeline import run_analyze_workflow
    ...
    @pytest.mark.asyncio
    @patch("betting.analyze.pipeline.save_active_bets")
    @patch("betting.analyze.pipeline.load_games_for_date", return_value=[])
    @patch("betting.analyze.pipeline.get_active_bets")
...
class TestPipelineEarlyExits:
    ...
    @pytest.mark.asyncio
    @patch("betting.analyze.pipeline.get_active_bets", return_value=[])
    @patch("betting.analyze.pipeline.load_games_for_date", return_value=[])
    ...
    @pytest.mark.asyncio
    @patch("betting.analyze.pipeline.fetch_polymarket_prices")
    @patch("execution.gamma.fetch_nba_events", return_value=[])
    @patch("betting.analyze.pipeline._extract_and_compute_injuries", new_callable=AsyncMock)
    @patch("betting.analyze.pipeline._enrich_games_with_search", new_callable=AsyncMock)
    @patch("betting.analyze.pipeline.get_active_bets", return_value=[])
    @patch("betting.analyze.pipeline.load_games_for_date")
...
class TestPipelineNoBalance:
    ...
    @pytest.mark.asyncio
    @patch("betting.analyze.pipeline.run_paper_trades", new_callable=AsyncMock)
    @patch("betting.analyze.pipeline.save_skips")
    @patch("betting.analyze.pipeline.get_polymarket_balance", return_value=None)
    @patch("betting.analyze.pipeline.synthesize_bets", new_callable=AsyncMock)
    @patch("betting.analyze.pipeline.analyze_game", new_callable=AsyncMock)
    @patch("betting.analyze.pipeline.read_text", return_value=None)
    @patch("betting.analyze.pipeline.get_history", return_value={"bets": [], "summary": {}})
    @patch("betting.analyze.pipeline.fetch_polymarket_prices")
    @patch("execution.gamma.fetch_nba_events", return_value=[])
    @patch("betting.analyze.pipeline._extract_and_compute_injuries", new_callable=AsyncMock)
    @patch("betting.analyze.pipeline._enrich_games_with_search", new_callable=AsyncMock)
...
class TestPipelineFullFlow:
    ...
    @pytest.mark.asyncio
    @patch("betting.analyze.pipeline._run_props_pipeline", new_callable=AsyncMock)
    @patch("betting.analyze.pipeline.get_dollar_pnl", return_value=0.0)
    @patch("betting.analyze.pipeline.write_journal_pre_game")
    @patch("betting.analyze.pipeline.run_paper_trades", new_callable=AsyncMock)
    @patch("betting.analyze.pipeline.save_skips")
    @patch("betting.analyze.pipeline.save_active_bets")
    @patch("betting.analyze.pipeline.size_bets", new_callable=AsyncMock)
    @patch("betting.analyze.pipeline.get_polymarket_balance", return_value=500.0)
    @patch("betting.analyze.pipeline.synthesize_bets", new_callable=AsyncMock)
    @patch("betting.analyze.pipeline.analyze_game", new_callable=AsyncMock)
    @patch("betting.analyze.pipeline.read_text", return_value="## Strategy")
    @patch("betting.analyze.pipeline.get_history", return_value={"bets": [], "summary": {}})
    @patch("betting.analyze.pipeline.fetch_polymarket_prices")
    @patch("execution.gamma.fetch_nba_events", return_value=[])
    @patch("betting.analyze.pipeline._extract_and_compute_injuries", new_callable=AsyncMock)
...
class TestPipelineBetFiltering:
    ...
    @patch("betting.analyze.pipeline.get_active_bets", return_value=[])
    @patch("betting.analyze.pipeline.load_games_for_date")
    @patch("betting.analyze.pipeline._extract_poly_and_odds_price")
...
class TestGameDataLoading:
    ...
    def test_loads_matching_files(self, tmp_path):
        from betting.analyze.gamedata import load_games_for_date

        # Create matching file
        game_data = {"matchup": {"team1": "A", "team2": "B"}}
        (tmp_path / "12345_2026-02-21.json").write_text(json.dumps(game_data))

        with patch("betting.analyze.gamedata.OUTPUT_DIR", tmp_path):
...
class TestSkipPersistence:
    ...
    def test_save_skips_replaces_for_date(self, tmp_path):
        from betting.io import save_skips, get_skips, SKIPS_PATH

        skips_path = tmp_path / "skips.json"
        with patch("betting.io.SKIPS_PATH", skips_path), \
             patch("betting.io.read_json") as mock_read, \
             patch("betting.io.write_json") as mock_write:
...
class TestDateHandling:
    ...
    def test_extracts_dates_from_output_files(self, tmp_path):
        from betting.cli import get_dates_from_output, OUTPUT_DIR
