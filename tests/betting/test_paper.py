...
from betting.types import PaperTrade
...
from betting.prompts import (
    PAPER_TRADE_PROMPT,
    SYSTEM_PAPER_ANALYST,
    format_paper_trade_insights,
)
from betting.paper import run_paper_trades, create_paper_trade, MIN_PAPER_TRADES_FOR_STRATEGY
...
class TestPaperTradeIO:
    ...
    def test_get_paper_trades_empty(self, tmp_path):
        with patch("betting.io.PAPER_DIR", tmp_path):
...
class TestRunPaperTrades:
    ...
    async def test_no_skips_returns_early(self):
        ...
        with patch("betting.paper.complete_json", new_callable=AsyncMock) as mock_llm:
    ...
    async def test_produces_paper_trades(self):
        ...
        with patch("betting.paper.complete_json", new_callable=AsyncMock, return_value=llm_response), \
             patch("betting.paper.save_paper_trades") as mock_save, \
             patch("betting.paper.get_paper_trades", return_value=[]), \
             patch("betting.paper.read_text", return_value=None), \
             patch("betting.paper.get_paper_history", return_value={"trades": [], "summary": {"total_trades": 0}}), \
             patch("betting.paper.write_paper_journal") as mock_journal:
...
class TestResolvePaperTrades:
    def test_evaluate_moneyline_paper_trade(self):
        ...
        from betting.evaluation import _evaluate_bet
...
class TestCategorizeSkipReason:
    def test_injury_category(self):
        from betting.results.history import _categorize_skip_reason
...
class TestPaperStrategyUpdate:
    ...
    async def test_needs_minimum_trades(self):
        ...
        with patch("betting.paper.get_paper_history", return_value={
...
class TestPaperTradeIntegration:
    ...
    async def test_analyze_imports_paper_trades(self):
        ...
        from betting.analyze.pipeline import run_paper_trades as imported
...
class TestPaperInit:
    def test_init_creates_paper_directory(self, tmp_path):
        ...
        with patch("betting.init.BETS_DIR", tmp_path), \
             patch("betting.init.PAPER_DIR", paper_dir), \
             patch("betting.init.PAPER_JOURNAL_DIR", paper_journal_dir):
