...
class TestPolyAndOddsExtraction:
    ...
    @patch("betting.analyze.sizing.extract_poly_price_for_bet", return_value=0.60)
    @patch("betting.analyze.sizing.poly_price_to_american", return_value=-150)
...
class TestSizeBets:
    ...
    @pytest.mark.asyncio
    @patch("betting.analyze.sizing.get_open_exposure", return_value=0.0)
    @patch("betting.analyze.sizing.get_dollar_pnl", return_value=0.0)
    @patch("betting.analyze.sizing.complete_json")
    async def test_llm_amount_capped_at_kelly_120_percent(
        ...
        from betting.analyze.sizing import size_bets
    ...
    @patch("betting.analyze.sizing.get_open_exposure", return_value=0.0)
    @patch("betting.analyze.sizing.get_dollar_pnl", return_value=0.0)
    @patch("betting.analyze.sizing.complete_json", return_value=None)
    ...
    @pytest.mark.asyncio
    @patch("betting.analyze.sizing.get_open_exposure", return_value=200.0)
    @patch("betting.analyze.sizing.get_dollar_pnl", return_value=0.0)
    @patch("betting.analyze.sizing.complete_json", return_value=None)
