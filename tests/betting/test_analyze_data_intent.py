...
class TestAnalyzeGameDataHygiene:
    ...
    @pytest.mark.asyncio
    @patch("betting.analyze.pipeline.complete_json")
    async def test_strips_internal_keys_from_llm_payload(self, mock_llm):
        from betting.analyze.pipeline import analyze_game
    ...
    @pytest.mark.asyncio
    @patch("betting.analyze.pipeline.complete_json", return_value=None)
...
class TestJournalWriting:
    ...
    def test_writes_journal_with_bets(self, tmp_path):
        with patch("betting.analyze.bets.JOURNAL_DIR", tmp_path):
            bets = [{
