...
class TestTeamNameMatching:
    ...
    def test_exact_match(self):
        from betting.results.game_results import _teams_match
...
class TestGameResultParsing:
    ...
    def test_finished_game_parsing(self):
        from betting.results.game_results import parse_single_game_result
    ...
    def test_score_formatting(self):
        ...
        from betting.results.game_results import _format_score
    ...
    def test_empty_api_response_returns_empty_list(self):
        from betting.results.game_results import parse_game_results
...
class TestPlayerStatLookup:
    ...
    def test_finds_player_points(self):
        from betting.evaluation import _find_player_stat
...
class TestPlayerPropVoidHandling:
    ...
    def test_dnp_player_is_voided(self):
        ...
        from betting.results.results import _resolve_bet_outcomes
        ...
        with patch("betting.results.results.get_game_player_stats", new_callable=AsyncMock, return_value=[]), \
             patch("betting.results.results.save_void") as mock_void:
    ...
    def test_unavailable_box_score_is_voided(self):
        ...
        with patch("betting.results.results.get_game_player_stats", new_callable=AsyncMock, return_value=None), \
             patch("betting.results.results.save_void") as mock_void:
    ...
    def test_unsupported_prop_type_is_voided(self):
        ...
        with patch("betting.results.results.get_game_player_stats", new_callable=AsyncMock, return_value=[{"player": {}}]), \
             patch("betting.results.results.save_void") as mock_void:
            matched, _ = asyncio.get_event_loop().run_until_complete(
                _resolve_bet_outcomes([bet], finished)
            )
    ...
    def test_invalid_game_id_is_voided(self):
        ...
        with patch("betting.results.results.save_void") as mock_void:
            matched, _ = asyncio.get_event_loop().run_until_complete(
                _resolve_bet_outcomes([bet], finished)
            )
    ...
    def test_successful_prop_bet_evaluation(self):
        ...
        with patch("betting.results.results.get_game_player_stats", new_callable=AsyncMock, return_value=box_score), \
             patch("betting.results.results.save_void"):
            matched, _ = asyncio.get_event_loop().run_until_complete(
                _resolve_bet_outcomes([bet], finished)
            )
    ...
    def test_box_score_cached_across_same_game(self):
        ...
        with patch("betting.results.results.get_game_player_stats", mock_stats), \
             patch("betting.results.results.save_void"):
            matched, _ = asyncio.get_event_loop().run_until_complete(
