"""Tests for betting/check.py — position re-evaluation workflow."""
...
class TestExecuteClose:
    def test_successful_close(self):
        ...
        with patch("betting.check.sell_position", return_value={"status": "ok"}), \
             patch("betting.check.get_history", return_value={"bets": [], "summary": {}}), \
             patch("betting.check.save_history") as mock_save, \
             patch("betting.check.update_history_with_bet") as mock_update:
    ...
    def test_sell_exception(self):
        ...
        with patch("betting.check.sell_position", side_effect=Exception("Network error")):
            result = execute_close(
                bet, pnl, recommendation,
...
class TestRunCheckWorkflow:
    @patch("betting.check.get_active_bets", return_value=[])
    ...
    @patch("betting.check.get_active_bets")
    ...
    @patch("betting.check.fetch_nba_events", return_value=[])
    @patch("betting.check.get_active_bets")
    ...
    @patch("betting.check.append_journal_check")
    @patch("betting.check.resolve_token_id", return_value=("token123", 0.50))
    @patch("betting.check.fetch_nba_events", return_value=[{"title": "test"}])
    @patch("betting.check.get_active_bets")
    ...
    @patch("betting.check.get_dollar_pnl", return_value=-40.0)
    @patch("betting.check.create_clob_client")
    @patch("betting.check.execute_close", return_value=True)
    @patch("betting.check.reevaluate_position")
    @patch("betting.check.search_position_context")
    @patch("betting.check.resolve_token_id", return_value=("token123", 0.30))
    @patch("betting.check.fetch_nba_events", return_value=[{"title": "test"}])
    @patch("betting.check.get_active_bets")
    ...
    @patch("betting.check.append_journal_check")
    @patch("betting.check.reevaluate_position", return_value=None)
    @patch("betting.check.search_position_context", return_value=None)
    @patch("betting.check.resolve_token_id", return_value=("token123", 0.30))
    @patch("betting.check.fetch_nba_events", return_value=[{"title": "test"}])
    @patch("betting.check.get_active_bets")
