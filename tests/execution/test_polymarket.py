import pytest
from execution.odds import american_to_implied_probability, format_price_comparison, poly_price_to_american
...
from execution.matching import (
...
from execution.gamma import fetch_nba_events, find_market
...
class TestFetchNbaEvents:
    @patch("execution.gamma.requests.get")
...
from execution.polymarket import resolve_token_id
...
class TestRun:
    @patch("execution.polymarket.save_active_bets")
    @patch("execution.polymarket.place_bet")
    @patch("execution.polymarket.fetch_nba_events")
    @patch("execution.polymarket.get_active_bets")
    def test_run_places_bets(self, mock_get_bets, mock_fetch, mock_place, mock_save):
        ...
        from execution.polymarket import run
        with patch.dict("os.environ", {
            "POLYMARKET_PRIVATE_KEY": "0x" + "ab" * 32,
            "POLYMARKET_FUNDER": "0x" + "cd" * 20,
        }):
            with patch("execution.polymarket.create_clob_client") as mock_client:
                run()
    ...
    def test_drift_gate_skips_drifted(self, mock_get_bets, mock_fetch, mock_place, mock_save):
        ...
        with patch.dict("os.environ", {
            ...
            with patch("execution.polymarket.create_clob_client"):
                run()
...
from execution.gamma import extract_polymarket_odds
