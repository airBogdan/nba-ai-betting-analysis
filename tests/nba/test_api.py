...
class TestComputeLeagueAvgEfficiency:
    ...
    async def test_computes_from_api(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "sports.nba.api.league.LEAGUE_EFFICIENCY_CACHE",
        ...
        with patch("sports.nba.api.league.fetch_nba_api", new_callable=AsyncMock, return_value=teams) as mock_teams, \
             patch("sports.nba.api.league.get_team_statistics", new_callable=AsyncMock) as mock_stats:
    ...
    async def test_reads_from_fresh_cache(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "sports.nba.api.league.LEAGUE_EFFICIENCY_CACHE", cache_file,
        )

        with patch("sports.nba.api.league.fetch_nba_api", new_callable=AsyncMock) as mock_api:
            result = await compute_league_avg_efficiency(2025)
    ...
    async def test_fallback_when_api_fails(self, tmp_path, monkeypatch):
        ...
        with patch("sports.nba.api.league.fetch_nba_api", new_callable=AsyncMock, return_value=None):
            result = await compute_league_avg_efficiency(2025)
    ...
    async def test_handles_corrupt_cache(self, tmp_path, monkeypatch):
        ...
        with patch("sports.nba.api.league.fetch_nba_api", new_callable=AsyncMock, return_value=teams), \
             patch("sports.nba.api.league.get_team_statistics", new_callable=AsyncMock, return_value=stats):
            result = await compute_league_avg_efficiency(2025)
...
class TestGetScheduledGamesETFilter:
    ...
    async def test_filters_by_et_date(self):
        ...
        with patch("sports.nba.api.games.get_games_by_date", side_effect=mock_get_games):
