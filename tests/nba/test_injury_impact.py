...
class TestExtractInjuriesFromSearch:
    ...
    async def test_successful_extraction(self):
        ...
        with patch("betting.analyze.injuries.complete_json", new_callable=AsyncMock, return_value=mock_result):
    ...
    async def test_handles_none_response(self):
        with patch("betting.analyze.injuries.complete_json", new_callable=AsyncMock, return_value=None):
            result = await _extract_injuries_from_search("ctx", "Team A", "Team B")
        assert result == []
    ...
    async def test_uses_haiku_model(self):
        with patch("betting.analyze.injuries.complete_json", new_callable=AsyncMock, return_value=[]) as mock_llm:
